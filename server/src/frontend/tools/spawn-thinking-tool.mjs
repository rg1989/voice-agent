...
export const spawnThinkingTool = {
  ...
  function: {
    ...
    description: '调用后台 Agent 完成任何需要真去做、去查、去核实的事：访问或操作用户环境、设备、文件、屏幕、应用和代码；检索当前信息（联网搜索、打开网页、查阅资料）；进行媒体创作、持续执行和制作交付物。也用于回答任何超出你既有知识、依赖最新数据或需要核实的问题 —— 与其凭训练数据作答或说自己查不到，不如交给它。还用于在用户补充信息、作出选择或确认后继续、修改已有工作。',
    parameters: {
      type: 'object',
      properties: {
        objective: {
